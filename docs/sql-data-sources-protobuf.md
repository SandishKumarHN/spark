---
layout: global
title: Apache Protobuf Data Source Guide
license: |
...
Since Spark 3.4 release, [Spark SQL](https://spark.apache.org/docs/latest/sql-programming-guide.html) provides built-in support for reading and writing Apache Protobuf data.
...
The Protobuf package provides function `to_protobuf` to encode a column as binary in Protobuf
...
* If the "value" field that contains your data is in Protobuf, you could use `from_protobuf()` to extract your data, enrich it, clean it, and then push it downstream to Kafka again.
* `to_protobuf()` can be used to turn structs into Protobuf messages. This method is particularly useful when you would like to re-encode multiple columns into a single one when writing data out to Kafka.
